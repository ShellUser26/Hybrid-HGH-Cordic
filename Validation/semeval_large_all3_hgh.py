...
LOCAL_MODEL_PATH = "./semeval_base_e_model/best_hardware_baseline"
...
def rtl_matmul_override(tensor1, tensor2, *args, **kwargs):
    ...
    if tensor1.dim() == 4 and tensor2.dim() == 4:
        if tensor1.shape[-1] == tensor1.shape[-2] and tensor1.shape[-1] == tensor2.shape[-2]:
            ...
            # CRITICAL FIX: unsqueeze(-3) perfectly aligns Seq_K for the dot product
...
def run_hardware_architectures():
    ...
    print("\n" + "="*70)
    print("      FINAL PTQ BENCHMARK (BERT-LARGE HARDWARE SIMULATION)")
    print("="*70)
